...
import {
    ...
    Wand2, BookOpen, Zap, Wind, Heart, Flame, Moon,
} from 'lucide-react';
...
interface VoiceControls {
    ...
    pitch: number;      // -10 – 10
    emotion: string;    // happy | sad | excited | calm | angry | fear | ''
    style: string;      // narration | conversational | newscast | poetry | ''
    model: string;
}
...
const SAMPLE_TEXTS = [
    ...
    "The quick brown fox jumps over the lazy dog. Testing, testing, one two three.",
    "Imagine a world where your voice can be everywhere at once. That world is here.",
    "Breaking news: AI voice cloning has reached a new frontier of quality and expressiveness.",
];
        ...
        <div className="space-y-2">
            ...
            <div className="relative h-5 flex items-center">
                ...
                <input
                    ...
                    style={{ zIndex: 1 }}
                />
                <div className="absolute w-4 h-4 rounded-full shadow-lg border-2 border-white transition-all" style={{ left: `calc(${pct}% - 8px)`, background: color }} />
            </div>
...
export default function VoiceCloner() {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file) return;
        if (!file.type.startsWith('audio/')) { setCloneError('Please upload an audio file (MP3, WAV, M4A, etc.)'); return; }
        if (file.size > 25 * 1024 * 1024) { setCloneError('Maximum file size is 25MB.'); return; }
        setUploadFile(file);
    ...
    const handleClone = async () => {
        ...
        if (!voiceName.trim()) { setCloneError('Please give your voice a name first.'); return; }
        ...
        try {
            ...
            if (!res.ok || !data.success) throw new Error(data.error || data.details?.message || 'Failed to clone voice');
    ...
    const handleTestVoice = async (voiceId: string) => {
        ...
        try {
            ...
            if (data.audioContent) {
                ...
                setCloneError('No audio returned. Check your API key.');
            }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            ...
            <div className="mb-8">
                ...
                <p className="text-theme-secondary mt-2">Clone your voice, then sculpt it — control tone, emotion, speed, and style.</p>
            </div>
                                ...
                                {tab === 'record' && (
                                    <div className="space-y-7">
                                        <p className="text-center text-sm text-theme-secondary">
                                            Record at least <span className="text-purple-400 font-bold">30 seconds</span> of clear natural speech. The more audio, the better the clone.
                                        </p>
                                        ...
                                        <div className="flex flex-col items-center gap-5">
                                            ...
                                            <div className="text-center">
                                                ...
                                                <p className="text-xs text-theme-secondary mt-1">
                                                    {isRecording ? '🔴 Recording — click to stop' : audioBlob ? '✅ Captured — ready to clone' : 'Click the mic to start'}
                                                </p>
                                                {recordingTime > 0 && recordingTime < 30 && !isRecording && (
                                                    <p className="text-xs text-amber-400 mt-1">⚠️ Recommended: at least 30 seconds for best quality</p>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between p-4 bg-card-hover rounded-2xl border border-theme">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-theme-secondary">{fmt(recordingTime)} · webm</p>
                                                    </div>
                                                </div>
                                    ...
                                    <div className="space-y-5">
                                        ...
                                        <div onDragOver={e => e.preventDefault()} onDrop={handleDrop}
                                            ...
                                            <p className="text-theme-secondary group-hover:text-theme-primary transition-colors">
                                                Drop your file here or <span className="text-purple-400 font-bold">browse</span>
                                            </p>
                    ...
                    {step === 'record' && hasAudio && (
                        <div className="bg-card-theme rounded-3xl border border-theme p-6 space-y-4">
                            ...
                            <textarea value={voiceDescription} onChange={e => setVoiceDescription(e.target.value)}
                                placeholder="Optional: describe this voice (used for Inworld metadata)" rows={2}
                            ...
                            <button onClick={handleClone} disabled={cloning || !voiceName.trim()}
                                ...
                                {cloning ? <><Loader2 className="w-5 h-5 animate-spin" /> Cloning your voice…</> : <><Sparkles className="w-5 h-5" /> Clone My Voice</>}
                            </button>
                            <p className="text-xs text-center text-theme-secondary/50">Processing takes ~30s · Your voice stays private</p>
                        </div>
                    )}

                    {/* ── Done / Test Voice ── */}
                    {step === 'done' && activeVoice && (
                        <div className="bg-card-theme rounded-3xl border border-green-500/20 overflow-hidden space-y-0">
                            {/* Header */}
                            <div className="p-6 border-b border-theme flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-green-500/20 to-emerald-500/20 border border-green-500/30 flex items-center justify-center flex-shrink-0">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                        ...
                                        <button onClick={() => copyId(activeVoice.voiceId)} className="p-1 rounded text-gray-600 hover:text-purple-400 transition">
                            ...
                            <div className="p-6 space-y-4">
                                    ...
                                    <div className="flex gap-1.5">
                                        {SAMPLE_TEXTS.map((t, i) => (
                                            <button key={i} onClick={() => { setTestText(t); setTestAudioSrc(null); }}
                                                title={t}
                                                className={`w-6 h-6 rounded-full text-[10px] font-bold transition-all border ${testText === t ? 'bg-purple-500 border-purple-500 text-white' : 'bg-card-hover border-theme text-theme-secondary hover:border-purple-500/40'}`}>
                                                {i + 1}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <textarea value={testText} onChange={e => { setTestText(e.target.value); setTestAudioSrc(null); }}
                                    rows={3} placeholder="Type something to hear it in your voice…"
                                    className="w-full bg-card-hover border border-theme rounded-2xl px-4 py-3 text-theme-primary text-sm focus:outline-none focus:border-purple-500/50 resize-none" />
                                        ...
                                        <div className="p-5 space-y-5">
                                            ...
                                            <div className="grid grid-cols-2 gap-5">
                                                <RangeSlider label="Speed" value={controls.speed} min={0.5} max={2.0} step={0.05}
                                                    format={v => `${v.toFixed(2)}×`} onChange={v => patchControls('speed', v)} color="#a855f7" />
                                                <RangeSlider label="Pitch" value={controls.pitch} min={-10} max={10} step={0.5}
                                                    format={v => v > 0 ? `+${v}` : `${v}`} onChange={v => patchControls('pitch', v)} color="#06b6d4" />
                                            </div>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {EMOTIONS.map(e => (
                                                        <button key={e.value} onClick={() => patchControls('emotion', e.value)}
                                                            className={`px-3 py-1.5 rounded-xl text-xs font-bold border flex items-center gap-1.5 transition-all ${controls.emotion === e.value ? 'bg-purple-500/20 border-purple-500/40 text-purple-300' : 'bg-card-hover border-theme text-theme-secondary hover:border-purple-500/30'}`}>
                                            ...
                                            {/* Style */}
                                            <div>
                                                <p className="text-xs text-gray-400 font-medium mb-2">Speaking Style</p>
                                                <div className="flex flex-wrap gap-2">
                                                    {STYLES.map(s => (
                                                        <button key={s.value} onClick={() => patchControls('style', s.value)}
                                                            className={`px-3 py-1.5 rounded-xl text-xs font-bold border flex items-center gap-1.5 transition-all ${controls.style === s.value ? 'bg-cyan-500/20 border-cyan-500/40 text-cyan-300' : 'bg-card-hover border-theme text-theme-secondary hover:border-cyan-500/30'}`}>
                                                            {s.icon} {s.label}
                                                        </button>
                                                    ))}
                                                </div>
                                            ...
                                            {/* Model */}
                                            <div>
                                                <p className="text-xs text-gray-400 font-medium mb-2">AI Model</p>
                                                <div className="flex gap-2 flex-wrap">
                                                    {MODELS.map(m => (
                                                        <button key={m.value} onClick={() => patchControls('model', m.value)}
                                                            className={`px-3 py-1.5 rounded-xl text-xs font-bold border flex items-center gap-2 transition-all ${controls.model === m.value ? 'bg-amber-500/20 border-amber-500/40 text-amber-300' : 'bg-card-hover border-theme text-theme-secondary hover:border-amber-500/30'}`}>
                                                            {m.label}
                                                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${controls.model === m.value ? 'bg-amber-500/30 text-amber-200' : 'bg-white/5 text-gray-500'}`}>{m.badge}</span>
                                                        </button>
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            {/* Reset controls */}
                                            <button onClick={() => { setControls(DEFAULT_CONTROLS); setTestAudioSrc(null); }}
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => testAudioSrc ? toggleTestAudio() : handleTestVoice(activeVoice.voiceId)}
                                        ...
                                        {synthesizing
                                            ...
                                            : testAudioSrc
                                                ...
                                                : <><Sparklees className="w-4 h-4" /> Generate Audio</>
                                        }
                                    </button>
                                    {testAudioSrc && (
                                        <>
                        ...
                        <div className="p-3 space-y-2 max-h-[460px] overflow-y-auto">
                            {savedVoices.length === 0 ? (
                                <div className="text-center py-10">
                                    ...
                                    <p className="text-xs text-theme-secondary/40">No voices yet.<br />Clone one to start.</p>
                                </div>
                            ) : savedVoices.map(voice => (
                                <div key={voice.voiceId}
                                    className={`p-4 rounded-2xl border group cursor-pointer transition-all ${activeVoiceId === voice.voiceId
                                        ? 'border-purple-500/40 bg-purple-500/10'
                                        : 'border-theme bg-card-hover hover:border-purple-500/20'}`}
                                    onClick={() => { setActiveVoiceId(voice.voiceId); setClonedVoice(voice); setStep('done'); setTestAudioSrc(null); }}>
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${activeVoiceId === voice.voiceId
                                                ? 'bg-gradient-to-br from-purple-500 to-pink-500'
                                                : 'bg-card-theme border border-theme'}`}>
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                {voice.description && <p className="text-[10px] text-theme-secondary truncate mt-0.5">{voice.description}</p>}
                    ...
                    <div className="bg-gradient-to-br from-purple-900/10 to-pink-900/5 rounded-3xl border border-purple-500/10 p-5 space-y-3">
                        ...
                        <ul className="space-y-2 text-xs text-theme-secondary">
                            {[
                                ...
                                'Read varied content (questions, statements, emphasis)',
...
// Quick alias fix — Sparkles is already imported, but the generate button uses `Sparklees` typo guard
function Sparklees(props: any) { return <Sparkles {...props} />; }
